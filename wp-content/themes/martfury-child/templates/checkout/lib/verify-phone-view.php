    ...
    if ($_POST['otp_code'] && $_POST['customer_mobile_phone'] && wp_verify_nonce($_POST['verify-phone-form-view-nonce'], 'verify_phone_form_view')) {
      ...
      if ($user && !empty($user)) {
        ...
        if (validPhoneNumberWithOtpCode($_POST['customer_mobile_phone'], $_POST['otp_code'])) {
          ...
          $errors = __('<strong>Mã xác thực</strong> không hợp lệ', 'thns');
        }
      }
    } else {
      $errors = __('<strong>Mã xác thực</strong> không hợp lệ', 'thns');
...
<div class="verify-phone-form">
  <h4><?php echo __('Tài khoản của bạn chưa được cập nhật số điện thoại. Vui lòng xác thực số điện thoại trước khi đặt hàng.', 'thns'); ?></h4>
  ...
  <form class="form-verify-phone" method="post">
    <div class="woocommerce-form-row woocommerce-form-row--first form-row">
      <label for="verify_customer_mobile_phone"><?php esc_html_e( 'Số điện thoại', 'thns' ); ?>&nbsp;<span class="required">*</span></label>
      <div class="phone-wrapper">
        ...
        <button class="btn-send-opt-verify" data-text-retry="<?php echo __('Gửi lại', 'thns'); ?>" data-text-retry-time="<?php echo __('Tối đa '.  $config_otp_max_send .' lần/ngày', 'thns'); ?>" data-text="<?php echo __('Gửi xác thực', 'thns'); ?>"><?php echo __('Gửi xác thực', 'thns'); ?></button>
      </div>
    </div>
    <p class="woocommerce-form-row woocommerce-form-row--last form-row">
      <label for="otp_code"><?php esc_html_e( 'Mã xác thực', 'thns' ); ?>&nbsp;<span class="required">*</span></label>
    ...
    <p class="woocommerce-form-row woocommerce-form-row--last form-row">
      ...
      <input type="submit" class="woocommerce-Button button" name="verify_phone" value="<?php esc_html_e( 'Xác nhận', 'thns' ); ?>"/>
    </p>
  </form>
</div>
<script>
