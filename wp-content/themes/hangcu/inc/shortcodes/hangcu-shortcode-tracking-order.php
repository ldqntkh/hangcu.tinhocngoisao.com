 ...
 */
class CT_Shortcode_Order_Tracking {
	...
	public static function output( $atts ) {
		...
		if ( isset( $_REQUEST['orderid'] ) && wp_verify_nonce( $nonce_value, 'woocommerce-order_tracking' ) ) { // WPCS: input var ok.
			...
			} elseif ( ! $order_phone || empty(preg_match('/(09|03|07|08|05)+([0-9]{8}$)/',$order_phone )) ) {
				wc_print_notice( __( 'Vui lòng nhập số điện thoại hợp lệ', 'woocommerce' ), 'error' );
			} else {
