	...
	<div id="comments">
		...
		<?php 
			if( $hascmt ) {
				...
				$tag_id = isset($_GET['tag_id']) ? base64_decode($_GET['tag_id']) : '';
				foreach ($cat as $categoria) {
					...
					if( $comment_tags ) {
						...
						if( count($comment_tags) > 0 ) :
						global $wp;
						$request = home_url( $wp->request );
						echo '<p id="cmt-lst-tags" class="cmt-lst-tags"><label>Xem nhanh theo tags:</label>';
						echo ' <a href="'.$request .'#cmt-lst-tags'.'">Tất cả</a>';
						foreach( $comment_tags as $cmt ) { ?>
							-<a style="<?php if($tag_id == $cmt) echo 'color: red' ?>" href="<?= $request . '/?tag_id=' . base64_encode($cmt) . '#cmt-lst-tags' ?>"> <?= $cmt ?></a>
						<?php }
						echo '</p>';
						endif;
						break;
