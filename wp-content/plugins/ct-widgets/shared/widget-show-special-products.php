 ...
 */
if ( ! class_exists( 'HC_Special_Products' ) ) {
    /**
     * Class for adding widget
     * @since 1.0.0
     */
    class HC_Special_Products extends WP_Widget {
        ...
        function __construct() {
            ...
            parent::__construct(
                ...
                esc_html__('CT Special Product', 'hangcu'),
                /*Widget description*/
        ...
        public function form( $instance ) {
            ...
            <p>
				<label>Ids sản phẩm đặc biệt muốn hiển thị (phân cách bằng dấu phẩy)</label>
				<input class='widefat' type='text' name='<?= $this->get_field_name('special_product_ids') ?>' value='<?= $special_product_ids ?>' />
			</p>
            <p>
        ...
        public function update( $new_instance, $old_instance ) {
            ...
            $instance['special_product_ids'] = strip_tags($new_instance['special_product_ids']);
        ...
        public function widget($args, $instance) {
            ...
            $exclude_pr_ids = array();
            if( !empty( $special_product_ids) ) {
                $pr_ids = explode( ',',  $special_product_ids );
                for( $i = 0; $i < count( $pr_ids ); $i++ ) {
                    $exclude_pr_ids[] = intval($pr_ids[$i]);
                }
            }
	        $query_args = array(
				...
				<asside id='widget_hangcu_list_product' class="special_block <?= $hc_widget_class ?>" style="<?= $custom_css ?>">
					<div class="list_product">
						...
						<?php 
                            ...
                            // show product
                            /**
                             * 1 special product sẽ có 3 sản phẩm thường
                             */
                            $output_special_pd = array();
                            for( $i = 0; $i < count($exclude_pr_ids); $i++ ) {
                                $product = wc_get_product( $exclude_pr_ids[$i] );
                                if( $product ) :
                                ...
                                <div class="special-product <?php echo esc_attr( $hc_list_classes ); ?>">
									<ul class="post-container products">
										<?php
											...
											wc_get_template_part( 'content', 'product-special-display' );
                            ...
                            $index_display = 0;
                            if( count( $output_special_pd ) > 0 ) {
                                for( $i = 0; $i < count($output_special_pd); $i++ ) {
                                    echo $output_special_pd[$i];
                                    if( $i == count($output_special_pd) - 1 ) {
                                        for( $m = $index_display; $m < count($output_pds); $m++ ) {
                                            echo $output_pds[$m];
                                        }
                                    } else {
                                        // hiển thị 3 sp
                                        for( $m = $index_display; $m < ($index_display + 3); $m++ ) {
                                            if( $m >= count($output_pds) ) break;
                                            echo $output_pds[$m];
                                        }
                                        $index_display += 3;
                                    }
