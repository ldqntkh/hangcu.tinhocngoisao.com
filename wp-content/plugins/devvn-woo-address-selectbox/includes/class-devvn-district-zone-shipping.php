...
class DevVn_District_Zone_Shipping extends WC_Shipping_Method {
	...
	function calculate_shipping( $package = array() ) {
        ...
        if($final_rate == 'shipdisable'){
			...
			$rate = array(
                'id'        => $this->id . "_" .  $this->instance_id,
                'label'     => (isset($this->shippingrate_title) && $this->shippingrate_title) ? $this->shippingrate_title : __('Miễn phí vận chuyển','devvn'),
                'cost'      => 0,
                'taxes'     => '',
                'calc_tax'  => 'per_order'
            );
	...
	function get_final_district_rate( $package = array() ) {
        ...
        $subtotal = apply_filters('devvn_district_zone_subtotal', $woocommerce->cart->subtotal);
		...
		if($cost_rate && !empty($cost_rate)){
		    ...
		    if($box_shipdisable == 1){
                ...
                $rate = apply_filters('devvn_district_zone_rate_modify', $rate);
