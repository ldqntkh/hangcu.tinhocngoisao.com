...
if ( ! class_exists( 'WC_Email_Customer_Cancelled_Order', false ) ) :
	...
	class WC_Email_Customer_Cancelled_Order extends WC_Email {
		...
		public function __construct() {
			...
			$this->title          = __( 'Hủy đơn hàng theo yêu cầu', 'hangcu' );
			$this->description    = __( 'Email này sẽ được gửi đi khi trước đó khách hàng đã có 1 yêu cầu hủy đơn hàng và được nhân viên bán hàng tiếp nhận xử lý', 'hangcu' );
			$this->template_html  = 'emails/customer-cancelled-order.php';
			$this->template_plain = 'emails/customer-cancelled-order.php';
			...
			add_action( 'woocommerce_order_status_pendingcancel_to_cancelled', array( $this, 'trigger' ), 10, 2 );
		...
		public function get_default_subject() {
			return __( '[{site_title}]: Đơn hàng #{order_number} đã hủy', 'hangcu' );
		}
