<?php
  function hangcu_validate_address_checkout() {
    ...
    foreach($keyMap as $key => $value) {
        if (empty($_POST[$key])) {
            $errors[$key] = __( '<strong>'.$value.'</strong> không được để trống', 'hangcu' );
        } else if (!is_string($_POST[$key])) {
            $errors[$key] = __( '<strong>'.$value.'</strong> không hợp lệ', 'hangcu' );
        }
    }

    if (!isset($errors['billing_phone']) && empty(preg_match('/(09|03|07|08|05)+([0-9]{8}$)/', $_POST['billing_phone']))) {
        $errors['billing_phone'] = __( '<strong>Số điện thoại</strong> không hợp lệ', 'hangcu' );
    }
