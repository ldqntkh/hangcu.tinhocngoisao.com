...
<?php 
    ...
    if ($currentUser->ID !== 0) {
        $otherAddr = get_user_meta( $currentUser->ID, 'thns_multiple_shipping_addresses', true );
    } else {
    ...
    WC()->customer->set_billing_first_name(wc_clean( $addressSelected['billing_first_name'] )); 
    $lastname = $addressSelected['billing_last_name'] ? wc_clean( $addressSelected['billing_last_name']) : wc_clean( $addressSelected['billing_first_name'] );
    ...
    WC()->customer->set_shipping_first_name(wc_clean(  $addressSelected['billing_first_name'] )); 
    WC()->customer->set_shipping_last_name( $lastname ); 
    ...
    do_action( 'thns_checkout_update_order_review', $addressSelected );
...
<div class="checkout-payment">
    <h2><?php echo __('3. Chọn hình thức thanh toán', 'thns') ?></h2>
    <div class="payment-details">
        ...
        <form name="checkout" method="post" class="checkout woocommerce-checkout" action="<?php echo esc_url( wc_get_checkout_url() ); ?>" enctype="multipart/form-data">
            <input type="hidden" name="billing_first_name" value="<?php echo $addressSelected['billing_first_name']; ?>" />
    ...
    <div class="order-details">
        <div class="order-address">
            <div class="head">
                <span><?php echo __('Giao hàng đến địa chỉ', 'thns'); ?></span>
                <a class="edit-order" href="<?php echo add_query_arg( 'step', 'shipping', get_permalink('checkout') ); ?>">Sửa</a>
            </div>
            <div class="address-content">
                <div class="info">
                    <p><strong><?php echo __('Họ tên:' , 'thns') ?></strong> <span><?php echo esc_html($addressSelected['billing_first_name']) . ' ' . esc_html($addressSelected['billing_last_name']); ?></span></p>
                    <p><strong><?php echo __('Số điện thoại:' , 'thns') ?></strong> <span><?php echo esc_html($addressSelected['billing_phone']); ?></span></p>
                    <p><strong><?php echo __('Địa chỉ:' , 'thns') ?></strong> <span><?php echo isset($addressSelected['full_address']) ? esc_html($addressSelected['full_address']) : ''; ?></span></p>
                </div>
            </div>
        </div>
        <div class="order-items">
            <div class="head">
                ...
                <a  class="edit-order" href="<?php echo esc_url( wc_get_cart_url() ); ?>"><?php echo __('Sửa', 'thns'); ?></a>
            </div>
            <div class="items">
            ...
            <div class="foot">
                <div class="order-totals">
                    ...
                    <div class="line">
                        <span style="min-width: 100px">Phụ phí:</span>
                        <strong>Nhân viên kinh doanh báo phí vận chuyển theo đơn hàng</strong>
                    </div>
